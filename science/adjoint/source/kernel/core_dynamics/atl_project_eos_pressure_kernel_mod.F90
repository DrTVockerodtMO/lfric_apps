...
module atl_project_eos_pressure_kernel_mod
  ...
  contains
    procedure, nopass :: atl_project_eos_pressure_code
  end type

  !---------------------------------------------------------------------------
  ! Contained functions/subroutines
  !---------------------------------------------------------------------------
  public :: atl_project_eos_pressure_code
  ...
  subroutine atl_project_eos_pressure_code( cell, nlayers,                      &
    ...
    real(kind=r_def) :: exner_eos, ls_exner_eos
    ...
    do k = nlayers - 1, 0, -1
      ...
      r_exner = 0.0_r_def
      do df = ndf_w3, 1, -1
        r_exner(:) = r_exner(:) + m3_inv(ik,df,:) * exner_e(df)
      end do
...
&ls_theta_vd_at_quad)
  ...
  end subroutine atl_project_eos_pressure_code

end module atl_project_eos_pressure_kernel_mod
