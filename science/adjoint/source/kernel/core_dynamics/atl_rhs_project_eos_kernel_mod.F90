...
!-----------------------------------------------------------------------------
!> @brief Computes the adjoint of the equation of state by Galerkin projection.
module atl_rhs_project_eos_kernel_mod

  use argument_mod,              only : arg_type, func_type,         &
                                        GH_FIELD, GH_READ,           &
                                        GH_READWRITE, GH_SCALAR,     &
                                        GH_REAL, ANY_SPACE_9,        &
  ...
  contains
    procedure, nopass :: atl_rhs_project_eos_code
  end type

  !---------------------------------------------------------------------------
  ! Contained functions/subroutines
  !---------------------------------------------------------------------------
  public :: atl_rhs_project_eos_code
...
!! @param[in]      wqp_v             Vertical quadrature weights
  subroutine atl_rhs_project_eos_code( nlayers,                                      &
    ...
    do k = nlayers - 1, 0, -1
      ...
      do qp2 = nqp_v, 1, -1
        do qp1 = nqp_h, 1, -1
          ...
          exner_quad = eos * ls_eos * onemk_over_k / ls_exner_quad
          theta_vd_quad = - ls_eos * eos / ls_theta_vd_quad
          rho_quad = - ls_eos * eos / ls_rho_quad
  ...
  end subroutine atl_rhs_project_eos_code

end module atl_rhs_project_eos_kernel_mod
